...
<?php
    ...
    $connection = new mysqli('localhost', 'root', '', 'City');
    ...
    {
        return htmlentities($unsafe_data);//charecter to string
    }
    ...
    function login($email_id_unsafe, $password_unsafe, $table = 'users')
    {
        ...
        if ($num_rows > 1) {
            ...
            if ($table == 'Citizen' || $table == 'Volunteer') {
                ...
                } elseif ($table == 'Citizen') {
                    $_SESSION['user-type'] = 'clerk';
                } else {
                   // $_SESSION['user-type'] = 'doctor';
                     // $_SESSION['userId']='$email_id';
                     $_SESSION['user_idl'] =$email_id;
					$_SESSION['user_passwordl'] = $password;
                                  header("patient_info.php");
    ...
    function register($email_id_unsafe, $password_unsafe, $full_name_unsafe, $speciality_unsafe = 'Volunteer', $table = 'Citizen')
    {
  ...
  function enter_patient_info($full_name_unsafe, $age_unsafe, $weight_unsafe, $phone_no_unsafe, $address_unsafe)
  {
    ...
    function appointment_booking($patient_id_unsafe, $specialist_unsafe, $medical_condition_unsafe,$Dr_name)
    {
        global $connection;
        ...
        $sql = "INSERT INTO appointments VALUES (NULL, $patient_id, '$specialist', '$medical_condition', '$Dr_name', NULL, 'no')";
...
function getListOfName($table)
    {
        global $connection;

        return $connection->query("SELECT fullname FROM $table;");
    }
